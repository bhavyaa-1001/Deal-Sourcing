...
import { ArrowLeft, CheckCircle2, FileDown, Layers, Sparkles, FolderCheck, AlertCircle, Ban } from 'lucide-react';
    ...
    <div className="flex flex-col gap-8">
      ...
      {shortlistedCompanies.length === 0 ? (
        ...
        <div className="w-full flex flex-col gap-4 text-left">
          ...
          {/* Floating Compare Action Bar below table */}
          {selectedForCompare.length >= 2 && (
            <div className="mt-6 p-5 border border-blue-200 dark:border-blue-900 bg-blue-50/50 dark:bg-blue-950/20 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4 animate-fadeIn">
              <span className="text-base text-slate-800 dark:text-slate-200 font-bold">
                {selectedForCompare.length} candidates selected for side-by-side comparison.
              </span>
              <Button
                variant="primary"
                onClick={() => setCompareModalOpen(true)}
                leftIcon={<Layers className="h-5 w-5" />}
                className="min-h-[46px] px-6 self-stretch sm:self-auto"
              >
